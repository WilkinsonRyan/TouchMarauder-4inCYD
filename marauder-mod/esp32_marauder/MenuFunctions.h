...
class MenuFunctions
{
  private:
    ...
    bool menu_swipe_suppress = false;  // true while a scroll-swipe is in progress
    int theme_node_index = -1;         // index of the Dark/Light toggle in settingsMenu
  ...
  public:
    ...
    void scrollMenu(int delta);  // move the visible window by delta rows (clamped)
    void drawScrollArrows();     // ▲▼ on the right edge when a menu overflows
    void redrawButtonBorder(int b, bool pressed);  // repaint outline over icon/label so side borders never clip
    void runTouchCalibration();  // tap-4-corners recalibration, stored to prefs
    void updateMarquees();       // horizontally scroll menu labels too wide to fit
