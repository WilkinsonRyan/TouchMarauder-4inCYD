#pragma once
  ...
  extern bool dark_mode;             // derived: true for any dark-bg theme (Dark/Hacker/Pride), false = Light
  enum { THEME_DARK = 0, THEME_LIGHT = 1, THEME_HACKER = 2, THEME_PRIDE = 3 };
  ...
  void saveDisplayPrefs();           // persist brightness + theme
  extern unsigned short touch_cal[5];// user touch calibration (0s until set)
  void saveTouchCal();               // persist touch_cal after calibrating
 ...
 //// BOARD FEATURES
    ...
    #define HAS_SD
    #define USE_SD
    //#define HAS_GPS   // CYD has no GPS module; GPS init hangs boot (nogps)
    //#define CYD_22CAP
    //#define CYD_24
    //#define CYD_24G
    //#define CYD_24CAP
  ...
  // --- 4" ST7796 CYD (CYD_35) tweaks ---------------------------------------
  // Full-width menu rows so any tap on a row selects it (not just the words),
  // and JetBrains Mono for the menu font. Placed last so it wins over the
  // board defaults resolved above.
  #if defined(CYD_35) || defined(CYD_35CAP)
    // Backlight pin for the sketch's PWM control (TFT_eSPI no longer defines it).
    #ifndef TFT_BL
      #define TFT_BL 27
    #endif
    #undef KEY_X
    #undef KEY_W
    #define KEY_X (TFT_WIDTH / 2)   // centre horizontally
    #define KEY_W TFT_WIDTH         // span the full screen width
    // JetBrains Mono 14pt with padded rows and a 2px black gap between the white
    // button outlines. ~14 rows fit; menus longer than that (Sniffers=17,
    // General=16) show the scroll arrows.
    #undef KEY_Y
    #undef KEY_H
    #undef KEY_SPACING_Y
    #undef BUTTON_SCREEN_LIMIT
    #undef BUTTON_ARRAY_LEN
    #define KEY_Y 66                // first row centre, clears the top banner
    #define KEY_H 34                // row height: 14pt glyphs + roomy descender padding
    #define KEY_SPACING_Y 2         // black pixels between adjacent white outlines
    #define BUTTON_SCREEN_LIMIT 12  // rows visible before scrolling kicks in
    #define BUTTON_ARRAY_LEN BUTTON_SCREEN_LIMIT
    #undef MENU_FONT
    #define MENU_FONT &JetBrainsMono_Regular14pt7b
  #endif
